...
echo "$($PSQL "SELECT major FROM majors LEFT JOIN students ON majors.major_id = students.major_id WHERE students.major_id IS NULL OR first_name ILIKE '%ma%' ORDER BY major;")"
